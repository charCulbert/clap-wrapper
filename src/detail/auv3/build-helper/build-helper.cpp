...
struct auInfo
{
  ...
  void writePListFragment(std::ostream &of, int idx, const std::string &factoryClass) const
  {
    if (!clapid.empty())
    {
    ...
    of << "          <dict>\n"
       ...
       << "            <string>" << (factoryClass.empty() ? factoryBase() + std::to_string(idx) : factoryClass) << "</string>\n"
...
int main(int argc, char **argv)
{
  ...
  if (argc >= 4 && std::string(argv[argc - 2]) == "--factory-class")
  {
    factoryClass = argv[argc - 1];
    argumentCount -= 2;
  }
  ...
  else if (std::string(argv[1]) == "--fromclap")
  {
    ...
    auto mcode = nerr((idx < argumentCount) ? std::string(argv[idx++]) : std::string());
    auto mname = nerr((idx < argumentCount) ? std::string(argv[idx++]) : std::string());

    auto itype = nerr((idx < argumentCount) ? std::string(argv[idx++]) : std::string());
    auto isubt = nerr((idx < argumentCount) ? std::string(argv[idx++]) : std::string());
  ...
  std::string principalClass = factoryClass.empty() ? units[0].factoryBase() + "0" : factoryClass;
  ...
  for (const auto &u : units)
  {
    std::cout << "    + " << u.name << " (" << u.type << "/" << u.subt << ") by " << u.manunm << " ("
              << u.manu << ")" << std::endl;
    u.writePListFragment(of, idx++, factoryClass);
  }
  of << "        </array>\n";
  ...
  // --- Generate generated_auv3_entrypoints.hxx ---
  {
    ...
    if (factoryClass.empty()) cppf << "#import \"detail/auv3/auv3_audiounit.h\"\n\n";

    idx = 0;
    for (const auto &u : units)
    {
      if (!factoryClass.empty()) break;

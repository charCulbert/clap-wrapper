...
bool WrapAsAUV2::activateCLAP()
{
  ...
  const bool started = _parameterFlushLifecycle.activateAndStart(
      [&]
      {
        ...
        _processAdapter->setupProcessing(Inputs(), Outputs(), _plugin->_plugin,
                                         _plugin->_ext._params, this, &_parametertree, this,
                                         maxSampleFrames, _midi_preferred_dialect,
                                         _midi_supported_dialects, clapAudioInputs,
                                         clapAudioOutputs);
  ...
  _initialized.store(started, std::memory_order_release);
  return started;
}
...
void WrapAsAUV2::deactivateCLAP()
{
  ...
  // Unconditional: the blocks must be released even when there is no plugin or
  // the wrapper was never initialized.
  if (auto blk = _midioutput_hosteventlistblock.exchange(nullptr)) Block_release(blk);
  for (auto blk : _retiredEventListBlocks) Block_release(blk);
  _retiredEventListBlocks.clear();
#endif
    ...
    case CLAP_EVENT_NOTE_EXPRESSION:
    {
      ...
      if (ClapWrapper::detail::shared::noteExpressionToMidi1(event.noteexpression, bytes) > 0)
      {
        ...
        for (auto &i : _midi_outports)
        {
          if (i->_info.id == portid)
          {
            i->addMIDI3Byte(bytes, event.header.time);
            break;
          }
        }
      }
